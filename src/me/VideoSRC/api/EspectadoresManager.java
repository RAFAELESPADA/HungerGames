...
public class EspectadoresManager implements Listener {
	...
	public void onInteracts(PlayerInteractEvent e) {
		Player p = e.getPlayer();
		if ((p.getItemInHand().getType() == Material.FLINT_AND_STEEL) && (Main.Watch.contains(p.getName()))) {
			e.setCancelled(true);
			return;
		}
	}
	...
	public void Espectador(PlayerInteractEvent e) {
		Player p = e.getPlayer();
		if ((p.getItemInHand().getType() == Material.SLIME_BALL) && (Main.Watch.contains(p.getName()))
				&& ((e.getAction() == Action.RIGHT_CLICK_BLOCK) || (e.getAction() == Action.RIGHT_CLICK_AIR))) {
			Inventory inv = Bukkit.createInventory(p, 54, "§aPlayers");
			for (Player players : Bukkit.getOnlinePlayers()) {
				if (!players.getName().equals(p.getName())) {
					ItemStack skull = new ItemStack(Material.PLAYER_HEAD);
					SkullMeta sm = (SkullMeta) skull.getItemMeta();
					if ((!Main.Watch.contains(sm.getOwner())) && (!Main.Jogadores.contains(p.getName()))
							&& (!CMDAdmin.admin.contains(p.getName()))) {
						...
						lore.add("§AKit: §7" + KitName);
						lore.add("§AKills: §7" + CMDListener.getKs(players));
						sm.setLore(lore);
	...
	public void onChat(PlayerChatEvent e) {
		...
		if (Main.Watch.contains(p.getName())) {
			e.setCancelled(true);
			p.sendMessage("§eYou cant talk right now!");
			return;
		}
	}
	...
	public void CancelarKit(PlayerCommandPreprocessEvent event) {
		if ((Main.Watch.contains(event.getPlayer())) && (event.getMessage().toLowerCase().startsWith("/kit "))) {
			event.setCancelled(true);
			return;
		}
	}
