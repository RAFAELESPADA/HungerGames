...
public class CMDJogo implements CommandExecutor {
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("game") && Main.Partida) {
			p.sendMessage("§7Kills: §c" + CMDListener.getKs(p));
			p.sendMessage("§fTime: §7" + StringTimer.TimerGame(Main.Andamento));
			p.sendMessage("§7Players: §c" + Main.Jogadores.size());
